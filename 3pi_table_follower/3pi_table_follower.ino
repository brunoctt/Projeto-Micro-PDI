...
{
  if (!Serial.available()){
    Serial.write("Write com available");
  }
  else {
    Serial.println("Write sem available");
  }
    
  while (1)
  {
    if (!Serial.available()){
      Serial.println("not available dentro do while");
    }
    else {
      Serial.println("available dentro do while");
    } 
    follow_segment();
